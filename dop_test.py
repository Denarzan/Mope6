...
f_count = 0
for i in range(100):
    x1min, x2min, x3min = -30, 15, 20  # задані значення Xmin
    x1max, x2max, x3max = 20, 50, 35  # задані значення Xmax
    ...
    def func(x1, x2, x3): # задана функція
        coefficients = [8.6, 8.5, 7.9, 7.3, 0.2, 0.4, 6.1, 7.2, 0.8, 5.6, 9.3] 
...
#        print_equation(beta_coefficients, importance)
 ...
 #   print_matrix(m, N, natural_plan, y_arr, " для натуралізованих факторів:")
    ...
    for i in range(len(list(importance))):
        if list(importance)[i]:
            t_count += coefficients[i]
        else:
            f_count += coefficients[i]
    d = len(list(filter(None, importance)))
    fisher_criteria(m, N, d, natural_plan, y_arr, coefficients, importance)
print("Відношення середнього значення усіх значимих коефіцієнтів до не значимих: ", t_count/f_count)